...
function inspect() {
   ...
   */
  const ambiguous = hidraw.gadget.length > 0 && hidraw.physical.length > 0;
  ...
  if (!configured) {
    ...
    why = `both a USB gadget (${hidraw.gadget.length} interfaces) and a physical key ` +
      `(${hidraw.physical.length}) are on the bus. Clients take the first match and cannot ` +
      'tell them apart, and a WebAuthn ceremony would race both. Run one device at a ' +
      'time: unplug the key, or stop the gadget.';
