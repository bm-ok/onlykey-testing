...
class Watchdogs {
  ...
  _lastActivity() {
    const deviceOutput = this.device && this.device.log.lastAppendAt;
    return Math.max(this.lastProgress, deviceOutput || 0);
  }
...
async function run(opts = {}) {
  ...
  if (adapter !== 'emulated') {
    return finish('aborted',
      'the hardware adapter is a declared seam with a stub behind it - not implemented yet',
      EXIT.RUNNER_ERROR);
  }
  ...
  for (const file of files) {
    ...
    const outcome = await runOneFile({
      file, rel, section, meta, caps, runDir, runId, reporter, watchdogs, counts, opts,
    });
...
async function runOneFile(ctx) {
  const { file, rel, section, meta, caps, runDir, runId, reporter, watchdogs, counts, opts } = ctx;
  ...
  /* Each file boots from a fresh copy of the state it asked for. */
  ...
  const transport = new EmulatedTransport({ runDir: fileDir, storageDir, runId });
    ...
    return {
      stop: true,
        ...
        reason: `${watchdogs.fired.kind} watchdog: ${watchdogs.fired.reason}` +
          (watchdogs.fired.pending.length ? ` (pending: ${watchdogs.fired.pending.join('; ')})` : ''),
        code: EXIT.WATCHDOG,
      },
    };
  }
...
module.exports = { run, list, resolveTargets, listSections, EXIT };
