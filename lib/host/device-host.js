...
function parseArgs(argv) {
  const out = { socket: null, storage: null, markers: null, generation: 0 };
...
function main() {
  ...
  /* The bus-only detach the peer already handles; without a bridge there is
   * nothing to detach, but answering keeps the command from looking dropped. */
  ipc.on('set-plugged', () => ipc.publishPlugged(false));
