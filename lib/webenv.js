...
function create(device, opts = {}) {
  ...
  /*
   * kbpgp, on first use only - a two-megabyte browserify bundle, and loading it
   * eagerly would put a second on every webenv.create(), including the files
   * that only want the tunnel.
   *
   * The FACTORY, not an instance. plugin.js registers `kbpgp(false, console)`
   * for the pages to consume, but onlykey-pgp.js takes the factory and calls it
   * itself - twice, with different first arguments - so handing it an instance
   * fails with "imports.kbpgp is not a function".
   */
  let kbpgpFactory = null;
  Object.defineProperty(imports, 'kbpgp', {
    enumerable: true,
    get() {
      if (!kbpgpFactory) {
        kbpgpFactory = require(path.join(LIB_ROOT, 'onlykey', 'kbpgp-2.1.0.ok.ecc.js'));
      }
      return kbpgpFactory;
    },
  });
...
/**
 * The vendored PQC-aware openpgp.js fork.
 *
 * Not a module: it is a plain script assigning a global, so openpgp_loader.js
 * reads it as text through webpack's raw-loader and evaluates it. loadPlain()
 * knows that specifier, so this is just the loader run the ordinary way.
 */
function openpgp() {
  return loadPlain('openpgp_loader.js');
}
...
function loadPlain(moduleName) {
  ...
  const localRequire = (id) => {
    /*
     * webpack's raw-loader, which openpgp_loader.js uses to pull the vendored
     * fork in as TEXT: that file is a plain script (`var openpgp = (function
     * (exports) {...})({})`) with no module.exports, so it cannot be required -
     * it has to be read and evaluated. The loader's contract is an object with
     * the source on `.default`, and reproducing it is three lines rather than
     * a build step.
     */
    if (id.startsWith('raw-loader!')) {
      const target = id.slice('raw-loader!'.length);
      const file2 = target.startsWith('.') ? path.resolve(dir, target) : target;
      return { default: fs.readFileSync(file2, 'utf8') };
    }
...
module.exports = {
  create, load, loadPlain, openpgp, probe, vendoredVersions,
