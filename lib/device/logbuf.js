...
/**
 * Read a `byteprint()` dump that follows `label` in accumulated console text.
 *
 * The firmware dumps binary to the console in several places - the packet it is
 * about to hash, a derived private key, a computed public key - and always
 * through okcore.cpp's byteprint(), so there is one format to get right and it
 * has one dangerous property: `Serial.print(b, HEX)` does NOT zero-pad. 0x05
 * prints as "5" and 0x0A as "A", so a dump is a ragged mix of one- and
 * two-character tokens. Anything treating it as fixed-width hex shifts every
 * byte after the first small one and produces a plausible, wrong answer.
 *
 * Two further rules, both learned rather than assumed:
 *
 *   - Take the LAST occurrence. The console is an accumulator that tests
 *     deliberately do not clear, so a run has many dumps by the time anything
 *     reads one, and the interesting one is always the most recent.
 *   - Stop at the first token that is not one or two hex characters, not at the
 *     first non-hex LINE. "512" and "16" are real lines from this device's
 *     console and are perfectly good hex.
 *
 * Pinned by 00-sanity/06-primed-packet, away from any device.
 *
 * @param {string} text   accumulated console output
 * @param {string} label  the line the firmware prints immediately before the dump
 * @returns {Buffer|null} null when the label was never printed, or nothing
 *                        parseable followed it
 */
function byteprintAfter(text, label) {
  const at = String(text || '').lastIndexOf(label);
  if (at === -1) return null;

  const bytes = [];
  for (const token of text.slice(at + label.length).split(/\s+/)) {
    if (!token) continue;
    if (!/^[0-9A-Fa-f]{1,2}$/.test(token)) break;    // the dump has ended
...
module.exports = { LogBuffer, byteprintAfter };
