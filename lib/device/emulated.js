...
class EmulatedTransport extends EventEmitter {
  ...
  _onChildExit(child, code, signal) {
    ...
    const expected = fs.existsSync(marker);
    ...
    /*
     * The addon installs a segfault handler that writes "[okemu] FATAL:
     * segfault at" plus a backtrace to stderr before the process dies, so the
     * evidence is already captured; either the signal or that line is enough
     * to call it.
     */
    if (signal === 'SIGSEGV' || /\[okemu\] FATAL: segfault at/.test(this.stderr)) {
      this._declareFatal(FATAL.FIRMWARE_CRASH,
        `the firmware segfaulted (generation ${this.generation})`);
      return;
    }
    ...
    if (code === EXIT_MAPFAIL) {
       ...
       * because something else took the address first. A run legitimately
       * produces hundreds of restarts, so even a small per-boot failure rate
       * becomes a run-killer; retry, and count it so it stops being invisible
       * if the rate ever changes. In practice this should be rare: the
       * collision that used to crash-loop the daemon was the 32 MiB bit-band
       * alias fighting V8's ASLR'd heap, and that region is already optional
       * and skipped when its address is taken.
       */
      if (this.mapRetries < MAX_MAP_RETRIES) {
        this.mapRetries++;
        this.emit('map-retry', { attempt: this.mapRetries, generation: this.generation });
        this._spawn();
        return;
      }
      this._declareFatal(FATAL.MAP_FAILED,
        `the emulated flash could not be mapped after ${MAX_MAP_RETRIES} retries`);
      return;
    }

    if (signal === 'SIGKILL') {
      this._declareFatal(FATAL.HOST_KILLED,
        'the device host was SIGKILLed with no restart marker - an external or ' +
        'OOM kill, not a reboot');
      return;
    }

    this._declareFatal(FATAL.HOST_DIED,
      `the device host exited (code ${code}, signal ${signal}) without a restart marker`);
  }
...
module.exports = { EmulatedTransport, FATAL, BOOT_BANNER };
