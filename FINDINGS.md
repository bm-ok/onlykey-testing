...
| 10 | `setpqc` reported success for a load the device refused | python-onlykey | a client cannot detect it - there is no readback for a composite slot | **no** | fixed in `bm-ok` fork, PR pending | — |
