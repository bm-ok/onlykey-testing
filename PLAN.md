...
| section 2 | 48 passed, 0 failed (gadget) | skipped - `client-access`, the kit's adapter holds the nodes the CLI needs |
| section 3 | 72 passed, 0 failed (headless 50 + browser 22) | n/a - both tiers drive the emulator by design |
| **whole tree** | **238 passed** | **96 passed, 9 skipped-with-reason** |
...
**10 of 19 carried over, 2 of those partially.**
...
| cli | 2 | nothing - the section runs; these are now just tests to write |
...
- [ ] The one CLI row left: lib-agent GPG. `11-derived-xwing-cli` is done -
      `07-derived-xwing`, both transports into one derivation.
